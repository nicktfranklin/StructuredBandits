...
def run_save_models(model_matrix, name_tag, sample_kwargs=None):
    ...
    def sample_model(sampler, list_params, name=None):
        ...
        _params = pm.summary(trace)[list_params]
        _params['Model'] = name
        return _loo, _params
    ...
    _loo, model_params = sample_model(sample_hier_cls, bgp_params, 'Bayesian-GP')
    ...
    rbf_cls_params = ['mu_beta_rbf_mean', 'mu_beta_rbf_stdv', 'mu_beta_cls_mean', 'mu_beta_cls_stdv', 'mu_beta_stick',
                      'sigma_rbf_means', 'sigma_rbf_stdev', 'sigma_cls_means','sigma_cls_stdev', 'sigma_stick']
    _loo, model_params = sample_model(sample_hier_cls, rbf_cls_params, 'GP-RBF/Clustering')
    ...
    lin_cls_params = ['mu_beta_lin_mean', 'mu_beta_lin_stdv', 'mu_beta_cls_mean', 'mu_beta_cls_stdv', 'mu_beta_stick',
                      'sigma_lin_means', 'sigma_lin_stdev', 'sigma_cls_means', 'sigma_cls_stdev', 'sigma_stick']
    _loo, model_params = sample_model(sample_hier_cls, lin_cls_params, 'Linear/Clustering')
    ...
    lin_cls_params = ['mu_beta_rbf_mean', 'mu_beta_rbf_stdv', 'mu_beta_kal_mean', 'mu_beta_kal_stdv', 'mu_beta_stick',
                      'sigma_rbf_means', 'sigma_rbf_stdev', 'sigma_kal_means', 'sigma_rbf_stdev', 'sigma_stick']
    _loo, model_params = sample_model(sample_hier_cls, lin_cls_params, 'Linear/Clustering')
    ...
    _loo, model_params = sample_model(sample_hier_cls, scram_params, 'Scrambled')
...
if __name__ == "__main__":
    ...
    exp_linear(debug=True, sample_kwargs=dict(init='adapt_diag'))
